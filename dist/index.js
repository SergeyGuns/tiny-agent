...
exports.SupervisorAgent = exports.DeveloperAgent = void 0;
...
// --- МОДУЛЬ ЛОГИРОВАНИЯ ---
class AgentLogger {
    logPath = path.resolve('session.log');
    clear() {
...
class WorkspaceManager {
    static makeSnapshot() {
        try {
            ...
            try {
                cp.execSync('git add . && git stash', { stdio: 'ignore' });
                cp.execSync('git stash apply', { stdio: 'ignore' });
            }
            catch { }
        }
    }
...
async function queryLLM(messages) {
    ...
    const response = await fetch(`${baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
            model: modelName,
            messages: messages,
            temperature: 0, // Важно для стабильности паттерна ReAct
        }),
    });
    if (!response.ok) {
...
3. runPackageScript[{"script": "имя"}]`
    ...
    async run(userQuery, feedback = '') {
        ...
        for (let step = 1; step <= this.maxSteps; step++) {
            ...
            if (!match)
                return true; // Финал работы разработчика
...
class SupervisorAgent {
    ...
    async evaluateSession(originalQuery) {
...
${sessionLogs}`;
        ...
        if (!match)
            return { decision: 'rollback', reason: 'Супервизор не смог принять форматное решение.' };
        const type = match[1];
...
async function startPipeline(userTask) {
